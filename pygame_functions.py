...
keydict = {"space": pygame.K_SPACE, "esc": pygame.K_ESCAPE, "up": pygame.K_UP, "down": pygame.K_DOWN,
           "left": pygame.K_LEFT, "right": pygame.K_RIGHT, "return": pygame.K_RETURN, "esc": pygame.K_ESCAPE,
           "a": pygame.K_a,
           "b": pygame.K_b,
           "c": pygame.K_c,
           "d": pygame.K_d,
...
class Background():
    ...
    def setTiles(self, tiles, x, y, scale):
        if type(tiles) is str:
            self.tiles = [[loadImage(tiles, x*scale, y*scale)]]
        self.bgWidth = x
        self.bgHeight = y
        self.stagePosX = (x*scale - x) //2 * (-1)
        self.stagePosY = (y*scale - y) //2 * (-1)
        self.scrollx = 0
        self.scrolly = 0
        self.tileWidth = self.tiles[0][0].get_width()
        self.tileHeight = self.tiles[0][0].get_height()
        screen.blit(self.tiles[0][0], [self.stagePosX, self.stagePosY])
    ...
    def scroll(self, x, y):
        ...
        self.scrollx += x
        self.scrolly += y
        if self.stagePosX < 0 and self.stagePosX > self.bgWidth - self.tileWidth and\
           self.stagePosY < 0 and self.stagePosY > self.bgHeight - self.tileHeight:
            screen.blit(self.tiles[0][0], [self.stagePosX, self.stagePosY])
            self.surface = screen.copy()
            return True
        else:
...
def loadImage(fileName, x=0, y=0, useColorKey=False):
    if os.path.isfile(fileName):
        ...
        ret_image = image
        if x > 0 and y > 0:
            ret_image = pygame.transform.scale(ret_image, (x, y))
        return ret_image
    else:
...
def setBackgroundImage(img, x=0, y=0, scale=1):
    global background
    background.setTiles(img, x, y, scale)
    if screenRefresh:
        updateDisplay()
    return background
